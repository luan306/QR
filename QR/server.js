...
// 📤 Upload Excel để import/update thiết bị
app.post("/api/devices/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    const conn = await getConnection(); // ✅ dùng getConnection

    for (let row of rows) {
      const name = row.Name || null;
      const qrCode = row.QR_Code || null;
      const departmentName = row.Department || null;
      const deviceTypeName = row.DeviceType || null;
    ...
    res.json({ success: true, message: "✅ Import thiết bị thành công!" });
  } catch (err) {
...
// Xuất file Excel mẫu để nhập thiết bị
app.get("/api/devices/template", checkAdmin, (req, res) => {
  ...
  res.download(filePath, "device_template.xlsx", () => {
    fs.unlinkSync(filePath); // xóa sau khi tải về
  });
});
